            ...
            @if (!empty($pdfs))
                @foreach ($pdfs as $pdf)
                    <tr>
                        ...
                        <td>{{ basename($pdf) }}</td>
                        <td>
                            <a href="{{ route('contabilidad.show', basename($pdf)) }}" target="_blank"><button class="btn btn-info">Abrir</button></a>
                            <form action="{{ route('contabilidad.destroy', basename($pdf)) }}" method="POST">
                                @method('DELETE')
                                @csrf
